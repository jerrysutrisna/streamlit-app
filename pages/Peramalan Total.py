...
if uploaded_file:
    try:
        ...
        if st.button("Predict"):
            ...
            # === Tambahan Fitur POWER BI Style ===

            # Filter Tahun dan Bulan untuk Visualisasi
            st.subheader("📅 Filter Data Prediksi")
            
            # Buat opsi tahun dan bulan
            unique_years = forecast_df["Tanggal"].dt.year.unique()
            unique_months = forecast_df["Tanggal"].dt.month_name().unique()
            
            selected_years = st.multiselect("Pilih Tahun:", options=sorted(unique_years), default=sorted(unique_years))
            selected_months = st.multiselect("Pilih Bulan:", options=unique_months, default=list(unique_months))
            
            # Filter berdasarkan pilihan
            filtered_forecast = forecast_df[
                (forecast_df["Tanggal"].dt.year.isin(selected_years)) &
                (forecast_df["Tanggal"].dt.month_name().isin(selected_months))
            ]
            ...
            # Filter lagi berdasarkan rentang tanggal
            filtered_forecast = filtered_forecast[
                (filtered_forecast["Tanggal"] >= date_range[0]) & 
                (filtered_forecast["Tanggal"] <= date_range[1])
            ]
            
            st.write(f"Menampilkan data prediksi dari **{date_range[0].strftime('%B %Y')}** sampai **{date_range[1].strftime('%B %Y')}**")
            
            # Expandable section untuk visualisasi
            with st.expander("📈 Visualisasi Prediksi Terfilter", expanded=True):
                fig_filtered = px.line(
                    filtered_forecast,
                    x="Tanggal",
                    y="Prediksi Jumlah",
                    title="Prediksi Jumlah Barang - Filtered View",
                    markers=True
                )
            ...
            # Tambahkan Anotasi Puncak Prediksi
            st.subheader("📍 Anotasi Titik Puncak Prediksi")
            
            if not filtered_forecast.empty:
                peak_row = filtered_forecast.loc[filtered_forecast['Prediksi Jumlah'].idxmax()]
                fig_peak = px.line(
                    filtered_forecast,
                    x="Tanggal",
                    y="Prediksi Jumlah",
                    title="Prediksi Jumlah Barang dengan Anotasi Puncak",
                    markers=True
                )
                fig_peak.add_annotation(
                    x=peak_row["Tanggal"],
                    y=peak_row["Prediksi Jumlah"],
                    text=f"Puncak: {peak_row['Prediksi Jumlah']:,}",
                    showarrow=True,
                    arrowhead=2,
                    arrowsize=1,
                    ax=0,
            ...
            # Heatmap Kalender
            st.subheader("📅 Heatmap Prediksi Bulanan")
            
            filtered_forecast["Bulan"] = filtered_forecast["Tanggal"].dt.strftime("%b")
            filtered_forecast["Tahun"] = filtered_forecast["Tanggal"].dt.year
            
            heatmap_data = filtered_forecast.pivot_table(
                index="Tahun",
                columns="Bulan",
            ...
            fig_heatmap.update_layout(
                title="Heatmap Prediksi per Bulan",
                xaxis_title="Bulan",
                yaxis_title="Tahun"
            )
